...
fn main() {
    ...
    println!(
        "Available: {} bytes = {} MB",
        mem.mem_available.unwrap_or(0),
        mem.mem_available.unwrap_or(0) / 1024 / 1024
    );
    ...
    for proc in all_processes().unwrap() {
        count += 1;
        if let Ok(p) = proc
            && let Ok(status) = p.status()
            && let Some(rss_kb) = status.vmrss
        {
    ...
    big_procs.sort_by_key(|a| std::cmp::Reverse(a.2));

    for (pid, name, rss) in big_procs.iter().take(30) {
        println!(
            "  PID {:6}: {:20} - RSS: {:>12} bytes = {:>6} MB",
            pid,
            name,
            rss,
            rss / 1024 / 1024
